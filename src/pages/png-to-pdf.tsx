...
export default function PNGToPDFTool() {
    ...
    const handleFilesAccepted = useCallback((newFiles: File[]) => {
        const imageFiles = newFiles.filter(file => file.type === 'image/png');
        ...
        <div className="max-w-6xl mx-auto px-4 py-12">
            <div className="mb-8 md:mb-12 text-center px-4">
                ...
                <p className="text-gray-400 font-mono text-sm md:text-base">
                    Convert PNGs to PDF documents. Rearrange them instantly via drag & drop.
                </p>
            </div>
                ...
                >
                    <div className="flex justify-between items-center mb-6 border-b border-white/10 pb-4">
                        <h3 className="text-lg font-bold text-white flex items-center gap-2">
                            <ImagePlus className="w-5 h-5 text-neon-green" />
                                            ...
                                            <Draggable key={`${file.name}-${index}`} draggableId={`${file.name}-${index}`} index={index}>
                                                {(provided, snapshot) => (
                                                    <motion.div
                                                        ...
                                                        ${snapshot.isDragging ? 'shadow-[0_0_20px_rgba(0,255,255,0.4)] scale-105 z-50 border-neon-green' : 'border-white/10 hover:border-white/30'}`}
                                                    >
                                                        {/* Thumbnail Preview */}
                                                        <img
